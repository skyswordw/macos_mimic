...
import { FaSearch, FaFolderOpen, FaSafari, FaTerminal, FaCalculator, FaRegStickyNote, FaCog, FaCode } from 'react-icons/fa'
...
const Spotlight = () => {
    ...
    return (
        <div className="absolute inset-0 z-50 flex justify-center pt-32" onClick={toggleSpotlight}>
            <div
                className="w-[600px] bg-white/80 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden border border-white/40 flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="h-16 flex items-center px-4 gap-4 border-b border-gray-200/50">
                    <FaSearch className="text-2xl text-gray-400" />
                    <input
                        ref={inputRef}
                        type="text"
                        placeholder="Spotlight Search"
                        className="flex-1 bg-transparent border-none outline-none text-2xl text-gray-700 placeholder-gray-400"
                        value={search}
                ...
                {filteredApps.length > 0 && (
                    <div className="py-2">
                        <div className="px-4 py-1 text-xs font-bold text-gray-500">Top Hit</div>
                        {filteredApps.map((app, i) => (
                            <div
                                key={app.id}
                                className={`px-4 py-2 flex items-center gap-3 cursor-pointer ${i === 0 ? 'bg-blue-500 text-white' : 'hover:bg-gray-200'}`}
